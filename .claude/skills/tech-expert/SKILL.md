---
name: tech-expert
description: Give the Tech Expert's ("dev expert") critique of a change, spec, or claim — code quality, maintainability, technical debt, and whether claims match the real current code. Use when the user asks for "the tech expert's view" or "the dev expert's view" on Opsmeld's products. For the full multi-role review, use the council skill instead. To write or scaffold code, use the dev skill.
---
